...
console.log(count); 
    ...
    .addSelect(
    `SUM(CASE WHEN IsOTPDeparture = 1 THEN 1 ELSE 0 END) AS IsOTPDeparture`
)
.addSelect(
    `SUM(CASE WHEN IsOTPArrival = 1 THEN 1 ELSE 0 END) AS IsOTPArrival`
)
// .addSelect(
//     `SUM(CASE WHEN IsOTPDeparture = 0 THEN 1 ELSE 0 END) + SUM(CASE WHEN IsOTPArrival = 0 THEN 1 ELSE 0 END) AS LateTotal`
// )
.addSelect(
    `SUM(CASE WHEN IsOTPDeparture = 1 THEN 1 ELSE 0 END) + SUM(CASE WHEN IsOTPArrival = 1 THEN 1 ELSE 0 END) AS TotalOTP`
)
.addSelect(`
    CASE 
...
.addSelect(`
    CASE 
        WHEN COUNT(ActualTimeDeparture) > 0 THEN 
            (SUM(CASE WHEN IsOTPDeparture = 1 THEN 1 ELSE 0 END) * 100.0) / COUNT(ActualTimeDeparture)
        ELSE 0 
    END AS PercentageDeparture
`)
.addSelect(`
    COUNT(ActualTimeDeparture) AS TotalDepartures,
    COUNT(ActualTimeOfArrival) AS TotalArrivals,
    CASE 
        WHEN COUNT(ActualTimeDeparture) > 0 OR COUNT(ActualTimeOfArrival) > 0 THEN 
            ROUND(
                (
                    (CASE 
                        WHEN COUNT(ActualTimeDeparture) > 0 THEN 
                            SUM(CASE WHEN IsOTPDeparture = 1 THEN 1 ELSE 0 END) * 1.0 / COUNT(ActualTimeDeparture) 
                        ELSE 0 
                    END) 
                    + 
                    (CASE 
                        WHEN COUNT(ActualTimeOfArrival) > 0 THEN 
                            SUM(CASE WHEN IsOTPArrival = 1 THEN 1 ELSE 0 END) * 1.0 / COUNT(ActualTimeOfArrival) 
                        ELSE 0 
                    END)
                ) * 100 / 2, 2)  -- Average of departure and arrival OTP %
        ELSE 0 
...
`)
    ...
    .addSelect("COUNT(EstimatedTimeDeparture) + COUNT(ActualTimeDeparture) AS TotalFlightsFlown")
   .addSelect(
    `SUM(CASE WHEN IsOTPDeparture = 1 THEN 1 ELSE 0 END) AS OnTimeDepartures`
)
.addSelect(
    `SUM(CASE WHEN IsOTPArrival = 1 THEN 1 ELSE 0 END) AS OnTimeArrivals`
)
.addSelect(
    `SUM(CASE WHEN IsOTPDeparture = 0 THEN 1 ELSE 0 END) + SUM(CASE WHEN IsOTPArrival = 0 THEN 1 ELSE 0 END) AS LateTotal`
)
.addSelect(
    `SUM(CASE WHEN IsOTPDeparture = 1 THEN 1 ELSE 0 END) + SUM(CASE WHEN IsOTPArrival = 1 THEN 1 ELSE 0 END) AS OnTimeTotal`
)
.addSelect(`
    CASE 
        WHEN COUNT(ActualTimeOfArrival) > 0 THEN 
...
.addSelect(`
    CASE 
        WHEN COUNT(ActualTimeDeparture) > 0 THEN 
            (SUM(CASE WHEN IsOTPDeparture = 1 THEN 1 ELSE 0 END) * 100.0) / COUNT(ActualTimeDeparture)
        ELSE 0 
    END AS PercentageOnTimeDeparture
`)
.addSelect(`
    COUNT(ActualTimeDeparture) AS TotalDepartures,
    COUNT(ActualTimeOfArrival) AS TotalArrivals,
